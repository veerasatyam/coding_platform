...
class Solution {
    ...
    private int helper(int[] coins, int amount, int index) {
        ...
        int take = 0;
        if (coins[index] <= amount)take = helper(coins,amount - coins[index],index);
        return notTake + take;
    }
}
...
class Solution {
    ...
    private int helper(int[] coins,int amount,int index,int[][] dp) {
        ...
        int take = 0;
        if (coins[index] <= amount)take = helper(coins,amount - coins[index],index,dp);
...
class Solution {
    public int change(int amount, int[] coins) {
        ...
        for (int t = 0;t<= amount;t++) if (t % coins[0] == 0) dp[0][t] = 1;
        for (int i = 1;i < n;i++){
